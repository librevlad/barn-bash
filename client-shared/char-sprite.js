...
(function (global) {
  ...
  const POSES = ['idle', 'move', 'dash', 'hit', 'teeter', 'cheer'];

  function _poseTransform(pose, clock) {
    // Returns { sx, sy, rot, dy } applied before drawing the sprite.
    switch (pose) {
      case 'move': {
        const bob = Math.sin(clock * 6);
        return { sx: 0.96 + bob * 0.04, sy: 1.04 - bob * 0.04, rot: bob * 0.04, dy: -Math.abs(bob) * 1.5 };
      }
      case 'dash': {
        // Strong forward stretch, low profile — aggressive lean.
        return { sx: 1.22, sy: 0.82, rot: 0, dy: 0 };
      }
      case 'hit': {
        // Flattened squash on impact.
        return { sx: 1.18, sy: 0.68, rot: 0.15, dy: 2 };
      }
      case 'teeter': {
        // Wobble — rapid shake simulating panicked balance.
        return {
          sx: 1,
          sy: 1,
      ...
      case 'cheer': {
        // Jumping pulse upward.
        const b = Math.abs(Math.sin(clock * 4));
        return { sx: 0.95 + b * 0.1, sy: 1.05 - b * 0.1, rot: 0, dy: -b * 10 };
      }
      case 'idle':
      default: {
        // Gentle breathing.
        const breath = Math.sin(clock * 1.8);
        return { sx: 1 + breath * 0.015, sy: 1 - breath * 0.015, rot: 0, dy: -breath * 0.5 };
      }
    }
  }
