...
HostHarness.boot({
  ...
  buildPostGameOpts: (state, msg) => {
    ...
    // Phase 52a — build per-player leaderboard. Finished racers
    // ranked by finishPos; unfinished ordered by waypoint desc then
    // lap desc. Value column shows FIN N for finished or Lap M.
    const entries = Object.entries(state.players || {}).map(([id, p]) => ({
      ...
      finished: p.finished,
      finishPos: (p.stats && p.stats.finishPos) || 0,
      lap: p.lap || 1,
      waypoint: p.waypoint || 0,
      stats: p.stats || {},
    }));
    entries.sort((a, b) => {
      ...
      if ((a.lap || 0) !== (b.lap || 0)) return (b.lap || 0) - (a.lap || 0);
      return (b.waypoint || 0) - (a.waypoint || 0);
    });
    const leaderboard = entries.map(e => ({
      ...
      score: e.finished ? ('#' + e.finishPos) : ('L' + e.lap),
      chips: [
        { label: 'Items', value: e.stats.itemsUsed || 0 },
        { label: 'Drifts', value: e.stats.drifts || 0 },
        { label: 'Bumps', value: e.stats.bumps || 0 },
      ],
    }));
