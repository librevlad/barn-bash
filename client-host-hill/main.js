...
// Phase 10c — async-strip lobby-hill backdrop white surround; swap
// img src to processed data URL once loadPainterly resolves.
setTimeout(() => {
  if (typeof SpriteLoader === 'undefined') return;
  SpriteLoader.loadPainterly('lobby-hill', '/assets/lobby-hill.png')
    .then((canvas) => {
...
function showWinner(winnerId) {
  ...
  if (p) {
    ...
    $winOverlay.innerHTML = `
      ...
      <div style="font-family:var(--font-display);font-size:22px;color:var(--accent-gold-hot);margin-top:6px;letter-spacing:3px;text-shadow:0 2px 0 var(--accent-red-deep)">IS KING!</div>
    ...
    $winOverlay.innerHTML = `
      <div class="w-text" style="color:var(--accent-gold-hot)">NOBODY SURVIVED!</div>
