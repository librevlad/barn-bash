...
function CharacterSelect({ onBack, onStart, playerCount=4, remotePlayers=null }) {
  ...
  // Adaptive slot width — keeps 6-8 players in a single row on a 1600-wide
  // stage without forcing a wrap into the ROSTER row below.
  const slotScale = slots.length <= 4 ? 1 : slots.length <= 6 ? 0.78 : 0.62;
  ...
  return (
    <div style={{position:'absolute',inset:0,background:'linear-gradient(180deg, #a6d8ee 0%, #c0e4f3 60%, #d9f0d0 100%)'}}>
      ...
      <div style={{position:'absolute', top: 24, left: 24, right:24, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        ...
        <div className="plank" style={{padding:'12px 30px', borderRadius:22, whiteSpace:'nowrap'}}>
      ...
      {/* Slot row */}
      <div style={{position:'absolute', top:140, left:0, right:0, display:'flex', justifyContent:'center', gap: slots.length > 6 ? 10 : slots.length > 4 ? 18 : 24, flexWrap:'nowrap', padding:'0 30px'}}>
        {slots.map((s, i) => (
          <CharSlot key={i} slot={s} idx={i} cycle={cycle} toggleReady={toggleReady} toggleCPU={toggleCPU} scale={slotScale} />
        ))}
      </div>
...
function CharSlot({ slot, idx, cycle, toggleReady, toggleCPU, scale = 1 }) {
  const isYou = idx === 0;
  ...
  const avatarSize = Math.round(110 * scale);
  return (
    <div className="pop-in" style={{
      background: slot.ready ? bgs[idx % bgs.length] : '#eadec0',
      border:'5px solid var(--ink)', borderRadius:20, padding:`${Math.round(14*scale)}px ${Math.round(16*scale)}px`,
      ...
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', fontFamily:"'Luckiest Guy'", fontSize: Math.round(16*scale), color:'var(--ink)'}}>
        ...
        {slot.displayName && !slot.isCPU ? (
          <span style={{
            ...
            borderRadius:8, padding:'2px 6px', fontFamily:"'Luckiest Guy'", fontSize: Math.round(11*scale),
          ...
          <button onClick={()=>toggleCPU(idx)} style={{
            ...
            borderRadius:8, padding:'2px 6px', fontFamily:"'Luckiest Guy'", fontSize: Math.round(11*scale), cursor:'pointer'
          }}>{slot.isCPU ? 'БОТ' : 'ЖИВОЙ'}</button>
        ))}
      </div>

      <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:8}}>
        <button onClick={()=>!slot.isCPU && !slot.displayName && cycle(idx, -1)} disabled={slot.isCPU || !!slot.displayName} style={{...arrowBtn, fontSize: Math.round(22*scale), padding:`${Math.round(6*scale)}px ${Math.round(10*scale)}px`}}>◀</button>
        <Avatar char={slot.char} size={avatarSize} bob/>
        <button onClick={()=>!slot.isCPU && !slot.displayName && cycle(idx, 1)} disabled={slot.isCPU || !!slot.displayName} style={{...arrowBtn, fontSize: Math.round(22*scale), padding:`${Math.round(6*scale)}px ${Math.round(10*scale)}px`}}>▶</button>
      </div>

      <div style={{textAlign:'center', fontFamily:"'Luckiest Guy'", fontSize: Math.round(22*scale), color:'var(--ink)', marginTop:4, lineHeight:1.1}}>
        {nameLabel}
      </div>
      <div style={{textAlign:'center', fontFamily:"'Luckiest Guy'", fontSize: Math.round(13*scale), color:'var(--wood-dk)', letterSpacing:.5, marginTop:2, marginBottom:8}}>
      ...
      <button onClick={()=>!slot.isCPU && !slot.displayName && toggleReady(idx)} disabled={slot.isCPU || !!slot.displayName} style={{
        ...
        fontFamily:"'Luckiest Guy'", fontSize: Math.round(18*scale), padding: Math.round(6*scale),
